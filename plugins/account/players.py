...
from hypernet import Region, EndfieldClient
...
class PlayersManagesPlugin(Plugin):
    ...
    async def refresh_cookies(self, update: "Update", _: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        if cookies.hg_token is not None:
            try:
                ...
                logger.error("刷新所有 token 失败 [%s]", (str(exc)))
                await callback_query.edit_message_text(
                    f"玩家 {player.player_id} {player_info.nickname} cookies 刷新失败 可能 hg_token 已经过期",
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
                return
