...
from hypernet.errors import DataNotPublic, BadRequest as SimnetBadRequest
...
class DailyNoteTasksPlugin(Plugin.Conversation):
    ...
    async def command_start(self, update: Update, _: CallbackContext) -> int:
        ...
        url = f"{config.pass_challenge_user_web}/tasks5?command=tasks&bot_data={note_user.web_config}"
    ...
    async def check_genshin_user(
        ...
        try:
            async with self.helper.genshin(user_id, player_id=uid, offset=offset) as client:
                ...
                if request_note:
                    note = await client.get_endfield_notes()
                    if max_resin and max_resin > note.max_stamina:
                        return f"设置失败，当前便签理智上限 {note.max_stamina} 超过设定值 {max_resin} ，请调整后再试。"
            ...
            if result.code == 0:
                if result.path == "tasks":
                    try:
                        ...
                        return ConversationHandler.END
                    need_note = await self.check_genshin_user(
                        validate.user_id, validate.player_id, None, True, validate.resin.notice_num
                    )
