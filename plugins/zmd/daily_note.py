...
from hypernet.errors import DataNotPublic
...
class DailyNotePlugin(Plugin):
    ...
    async def _get_daily_note(self, client: "EndfieldClient") -> RenderResult:
        daily_info = await client.get_endfield_notes(player_id=client.player_id)
        ...
        render_data = {
            ...
            "bp_current": daily_info.bp.current,
            "bp_total": daily_info.bp.total,
        }
        render_result = await self.template_service.render(
            "zmd/daily_note/daily_note.jinja2",
            render_data,
    ...
    async def command_start(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> Optional[int]:
        ...
        try:
            ...
            reply_message = await message.reply_text(
                "查询失败惹，可能是便签功能被禁用了？请尝试通过 APP 获取一次便签信息后重试。"
            )
