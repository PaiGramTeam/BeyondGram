...
from plugins.zmd.redeem.runner import RedeemRunner, RedeemResult, RedeemQueueFull
...
class Redeem(Plugin):
    ...
    def __init__(
        ...
        role_token_helper: RoleTokenHelper,
        ...
        self.redeem_runner = RedeemRunner(genshin_helper, role_token_helper)
    ...
    async def job_redeem_one_code(self, user_id: int, code: str, count: List[int], chinese: bool):
        task_data = RedeemResult(user_id=user_id, code=code, count=count)
        priority = 1 if await self.user_admin_service.is_admin(user_id) else 2
        try:
            await self.redeem_runner.run(task_data, self._job_callback, priority, True, chinese)
        except RedeemQueueFull:
            await sleep(5)
            await self.job_redeem_one_code(user_id, code, count, chinese)

    async def do_redeem_job(self, message: "Message", code: str, chinese: bool) -> Tuple[int, int]:
        count = [0, 0]
        region = RegionEnum.HYPERION if chinese else RegionEnum.HOYOLAB
        task_list = await self.cookies_service.get_all(region=region, status=CookiesStatusEnum.STATUS_SUCCESS)
    ...
    async def redeem_all_command_start(self, update: Update, context: CallbackContext) -> None:
        ...
        args = self.get_args(context)
        chinese = False
        if args and args[0].lower() == "cn":
            chinese = True
            args = args[1:]
        codes = [i for i in args if i]
        self.log_user(update, logger.info, "兑换码批量兑换命令请求 codes[%s] chinese[%s]", codes, chinese)
        if not codes:
            await message.reply_text("请输入兑换码")
            return
        code = codes[0]
        reply = await message.reply_text("开始运行批量兑换任务，请等待...")
        success, failed = await self.do_redeem_job(reply, code, chinese)
