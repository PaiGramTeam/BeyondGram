...
from hypernet.errors import BadRequest as SimnetBadRequest, InvalidCookies, TimedOut as SimnetTimedOut
...
class ResinData(TaskDataBase):
    notice_num: Optional[int] = 200

    @field_validator("notice_num")
    @classmethod
    def notice_num_validator(cls, v):
        if v < 50 or v > 300:
            raise ValueError("理智提醒数值必须在 50 ~ 300 之间")
        return v
...
class DailyNoteSystem(Plugin):
    ...
    @staticmethod
    def get_resin_notice(user: DailyNoteTaskUser, notes: "EndfieldNote") -> str:
        notice = None
        if user.resin_db and notes.max_stamina > 0:
            if notes.current_stamina >= user.resin.notice_num:
                if not user.resin.noticed:
                    notice = (
                        f"### 理智提示 ####\n\n当前理智为 {notes.current_stamina} / {notes.max_stamina} ，记得使用哦~\n"
    ...
    @staticmethod
    def get_daily_notice(user: DailyNoteTaskUser, notes: "EndfieldNote") -> str:
        notice = None
    ...
    async def start_get_notes(
        ...
        if not user:
            return []
        notes = await client.get_endfield_notes()
        notices = [
    ...
    async def update_task_user(self, user: DailyNoteTaskUser):
        if user.resin_db:
            try:
                ...
                logger.warning("用户 user_id[%s] 自动便签提醒 - 理智数据过期，跳过更新数据", user.user_id)
        if user.daily_db:
            try:
                await self.daily_service.update(user.daily_db)
            except StaleDataError:
                logger.warning("用户 user_id[%s] 自动便签提醒 - 活跃度数据过期，跳过更新数据", user.user_id)
