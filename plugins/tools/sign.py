...
from hypernet.errors import BadRequest as SimnetBadRequest, AlreadyClaimed, InvalidCookies, TimedOut as SimnetTimedOut
...
class SignSystem(Plugin):
    ...
    async def start_sign(
        ...
        calendar = rewards.calendar
        reward, reward_name, reward_amount = None, "unknown", 1
        has_available = any(day.available for day in calendar)
        claimed_rewards = len([day for day in calendar if day.done])
        for day in calendar:
            if has_available and day.available:
                reward = day.awardId
                break
            if not has_available and day.done:
                reward = day.awardId
        if reward_info := rewards.resourceInfoMap.get(reward):
            reward_name = reward_info.name
            reward_amount = reward_info.count

        today = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        cn_timezone = datetime.timezone(datetime.timedelta(hours=8))
        now = datetime.datetime.now(cn_timezone)
        missed_days = now.day - claimed_rewards
        if not rewards.hasToday:
            missed_days -= 1
        message = (
            f"#### {title} ####\n"
            ...
            f"今日奖励: {reward_name} × {reward_amount}\n"
